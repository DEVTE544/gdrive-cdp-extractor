...
CLONE_USER_DATA_DIR = Path(tempfile.gettempdir()) / "nve_cdp_clone"
...
def prompt_close_original_chrome():
    ...
    input(f"  {C.CYAN}{C.BOLD}Close original Chrome, then press Enter to continue ›{C.RESET}  ")
...
def clone_profile_to_workdir(force_rebuild=False):
    ...
    clone_root = CLONE_USER_DATA_DIR
...
def main():
    ...
    while True:
        ...
        # ── Single URL Mode ──────────────────────────────────────────────────
            ...
            try:
                asyncio.run(run(target_url, preferred_quality))
            except KeyboardInterrupt:
